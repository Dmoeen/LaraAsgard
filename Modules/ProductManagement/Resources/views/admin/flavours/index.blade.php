...
@section('content-header')
    <h1>
        {{ trans('productmanagement::flavours.title.flavours') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li class="active">{{ trans('productmanagement::flavours.title.flavours') }}</li>
    </ol>
@stop
        ...
        <div class="col-xs-12">
            <div class="row">
                <div class="btn-group pull-right" style="margin: 0 15px 15px 0;">
                    <a href="{{ route('admin.productmanagement.flavour.create') }}" class="btn btn-primary btn-flat" style="padding: 4px 10px;">
                        <i class="fa fa-pencil"></i> {{ trans('productmanagement::flavours.button.create flavour') }}
                    </a>
                </div>
            </div>
...
@section('shortcuts')
    <dl class="dl-horizontal">
        <dt><code>c</code></dt>
        <dd>{{ trans('productmanagement::flavours.title.create flavour') }}</dd>
    </dl>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'c', route: "<?= route('admin.productmanagement.flavour.create') ?>" }
                ]
            });
        });
    </script>
